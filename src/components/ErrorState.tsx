...
const ErrorState = ({
  ...
  const isDataStructureError =
    ...
    (rawResponse.includes('"programs_list":{}') ||
      rawResponse.includes('"programs_list":[]') ||
      rawResponse.includes('"content":{}') ||
      rawResponse.includes('"content":[]') ||
      validationErrors.some(
        (err) =>
          err.field.includes("programs_list") ||
  ...
  if (isCompanySummaryError) {
    formattedMessage = domainName
      ? `We couldn't generate a complete competitor analysis for "${domainName}". The company data returned was incomplete or invalid.`
      : "The competitor analysis data is incomplete or in an unexpected format. Please try generating a new analysis.";
    ...
    formattedMessage = domainName
      ? `We couldn't generate a competitor analysis for "${domainName}". The conversation ID appears to be invalid or the API couldn't process your request. Please try with a different domain.`
      : "We couldn't generate a competitor analysis. The conversation ID appears to be invalid. Please try starting a new request.";
    buttonText = "Try Different Domain";
    ...
    formattedMessage = `No competitor analysis data available for ${domainName}. This typically happens when there isn't enough public information available for the company. Please try a different domain.`;
    buttonText = "Try Different Domain";
    ...
    <Card className="w-full max-w-md shadow-md relative z-[1]">
      <CardHeader>
        ...
        <CardDescription className="text-center">
          Something went wrong while generating your competitor analysis
        </CardDescription>
      </CardHeader>
      <CardContent className="text-center space-y-4">
        <p className="text-gray-700 font-medium max-w-md">{formattedMessage}</p>
              ...
              <Alert variant="destructive" className="text-left text-xs">
                <AlertDescription>
                  <h4 className="font-medium mb-1">
                    The following data validation errors occurred:
                  </h4>
            ...
            <CollapsibleContent className="mt-2">
              ...
              {hasConversationIdInRawResponse && (
                <Alert variant="destructive" className="mt-2 text-xs">
                  <AlertDescription>
                    The API response contains a conversation ID field, but the
                    application may not be using it correctly. Please retry or
              ...
              {hasProgramTitleIssues && (
                <Alert variant="destructive" className="mt-2 text-xs">
                  <AlertDescription>
                    The API response contains program data but program titles
                    may be missing or in an unexpected format. This can cause
                    programs to display with generic names.
                  </AlertDescription>
                </Alert>
              )}

              {hasProgramDetailsInResponse &&
                !rawResponse.includes("program_name") && (
                  <Alert variant="destructive" className="mt-2 text-xs">
                    <AlertDescription>
                      Program details were found but program names may be
                      missing. This can affect how programs are displayed.
                    </AlertDescription>
                  </Alert>
                )}
            </CollapsibleContent>
          ...
          <Alert className="bg-amber-50 border-amber-200 text-amber-800 text-xs text-left">
            <AlertDescription>
              <p className="font-medium">
                The generated competitor analysis is missing required company
              ...
              <ul className="list-disc pl-5 mt-1 space-y-1">
                ...
                <li>
                  Our AI couldn't extract the necessary details from available
                  sources
                </li>
              </ul>
              <p className="mt-2">
                Please try generating a new competitor analysis with a different
                domain.
              </p>
            </AlertDescription>
          </Alert>
        ...
        {isDataStructureError && (
          <Alert className="bg-amber-50 border-amber-200 text-amber-800 text-xs text-left">
            <AlertDescription>
              <p className="font-medium">
                The generated analysis has data structure issues. This can
                happen if:
              </p>
              <ul className="list-disc pl-5 mt-1 space-y-1">
                <li>The API response format has changed</li>
                <li>
                  Program details weren't properly preserved during processing
                </li>
                <li>There was an issue with data mapping between systems</li>
              </ul>
              <p className="mt-2">
                Please try generating a new analysis with a different domain.
              </p>
            </AlertDescription>
          </Alert>
        )}
        ...
        {isNoConversationIdError && (
          <Alert className="bg-amber-50 border-amber-200 text-amber-800 text-xs text-left">
            <AlertDescription>
              <p className="font-medium">
                The conversation ID appears to be invalid or has expired. This
                can happen if:
              </p>
              <ul className="list-disc pl-5 mt-1 space-y-1">
            ...
            >
              ...
              <CollapsibleContent className="mt-2">
                <div className="text-xs text-left p-3 bg-amber-50 border border-amber-200 rounded">
                  <p>
                    <strong>Why this error occurs:</strong> Domains with certain
                    patterns (like short .io domains, test domains, or specific
                    TLDs) often don't have enough public information available
                    for our AI to generate a complete competitor analysis.
                  </p>
                  <p className="mt-2">
                    <strong>Recommendation:</strong> Try a more established
                    domain with substantial public web presence, such as a
                    company that has been operating for a longer period.
                  </p>
                  <p className="mt-2">
                    <strong>Examples of working domains:</strong> microsoft.com,
