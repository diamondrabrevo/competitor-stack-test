...
const LoadingState = ({
  ...
  useEffect(() => {
    console.log(
      `LoadingState rendered [${componentIdRef.current}]: domain=${companyDomain}, pollCount=${pollCount}, message=${message}`
    );
  ...
  useEffect(() => {
    ...
    // Log timer start
    console.log(
      `Starting timer [${componentIdRef.current}] from ${startTimeRef.current}`
    );
  ...
  useEffect(() => {
    if (pollCount !== undefined && pollCount > previousCountRef.current) {
      console.log(
        `Poll count updated [${componentIdRef.current}]: ${pollCount} (previous: ${previousCountRef.current})`
      );
  ...
  useEffect(() => {
    const jokeTimer = setInterval(() => {
      if (isMountedRef.current) {
        setCurrentJokeIndex(
          (prevIndex) => (prevIndex + 1) % marketingJokes.length
        );
      }
    }, 10000);
  ...
  useEffect(() => {
    return () => {
      ...
      [
        timerIdRef.current,
        jokeTimerIdRef.current,
        progressTimerIdRef.current,
      ].forEach((timer) => {
        if (timer) {
          clearInterval(timer);
        }
  ...
  const getStatusMessage = (): string => {
    ...
    if (elapsedTime < 60) {
      ...
      return "Finalizing your customized competitor analysis...";
    } else {
    ...
    <Card className="w-full max-w-lg shadow-md">
      <CardHeader className="pb-2">
        <CardTitle className="text-center text-2xl font-bold flex items-center justify-center text-green-500">
          ...
          Drafting your Competitor Analysis
        </CardTitle>
      ...
      <CardContent className="flex flex-col items-center gap-4 py-4">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-sm text-gray-500">
            Time elapsed: {formatTime(elapsedTime)} · Polls: {pollCount} ·
        ...
        <div className="text-center text-sm text-gray-600 mt-1 max-w-md">
          <p>
            We help marketers create beautiful customer experiences at Brevo.
            <br />
