 ...
 * before accessing the competitor analysis
 */
...
const EmailGateModal = ({
  ...
  const validateEmail = (
    ...
    if (!EMAIL_REGEX.test(emailAddress)) {
      return {
        isValid: false,
        errorMessage: EMAIL_GATE_MESSAGES.INVALID_EMAIL,
      };
    }
  ...
  const sendWebhook = async (userEmail: string) => {
    const webhookUrl =
      "https://hook.eu1.make.com/cs6y2v6qu0370u73mkkwp5s6kf3neoye";

    const payload = {
      email: userEmail,
      company_name: companyName,
      company_size: companySize || "Unknown",
      competitor_analysis_url: pageUrl,
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md relative overflow-hidden">
        <div className="p-6">
          <h2 className="text-xl font-semibold mb-3 text-gray-900">
            {EMAIL_GATE_MESSAGES.TITLE}
          </h2>
          <p className="mb-6 text-gray-600">
            {EMAIL_GATE_MESSAGES.DESCRIPTION}
          </p>
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-4">
              ...
              >
                {isSubmitting
                  ? "Processing..."
                  : EMAIL_GATE_MESSAGES.BUTTON_TEXT}
              </Button>
            </div>
          </form>
        </div>
